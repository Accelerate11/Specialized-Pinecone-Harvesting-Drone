#!/usr/bin/env python3
"""Evaluate a LocateAnything GGUF pair through llama-mtmd-cli."""
...
def summarize(counts: tuple[int, int, int] | list[int]) -> dict[str, float | int]:
    tp, fp, fn = counts
...
def main() -> int:
    ...
    for required in (args.runtime, args.model, args.mmproj, args.annotation, args.image_root):
        if not required.exists():
            parser.error(f"path does not exist: {required}")
    ...
    for position, (source_index, row) in enumerate(selected, start=1):
        ...
        target_boxes = parse_boxes(row["conversations"][1]["value"])
        command = [
            str(args.runtime),
            "-m", str(args.model),
        ...
        env = os.environ.copy()
        env["MTMD_GROUNDING_MODE"] = args.generation_mode
    ...
    result = {
        ...
        "wall_seconds_including_model_reload": round(time.perf_counter() - started, 3),
        ...
        "metrics_note": "Reference only: incomplete/noisy annotations can count plausible detections as false positives. Each sample reloads the model, so wall time is not persistent-process throughput.",
        "metrics": {str(threshold): summarize(counts) for threshold, counts in totals.items()},
        "records": records,
    }
